		...
		btnProcessing.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				while (flag) {
			        ...
			        for (int i = 0; i < list.length; i++) {                                                            //обход всего списка
			        	
			        	for (int j = 0; j + i < list.length; j++) {                                                    //сравнение 1 и 2  1 и 3  1 и 4
			        		
			        		for (int wordId = 0; wordId != list[i].length() && wordId != list[j].length(); wordId++) { //обход букв массива строк и проверка на выход из границ массива
			        			
			        			if (list[i].toCharArray()[wordId] < list[j + i].toCharArray()[wordId]) {               //если буква меньше то выход 
				        			break;
				        		}
				        		
			        			flag = false;
				        		if (list[i].toCharArray()[wordId] > list[j + i].toCharArray()[wordId]) {               //если буква больше то обмен указателей
