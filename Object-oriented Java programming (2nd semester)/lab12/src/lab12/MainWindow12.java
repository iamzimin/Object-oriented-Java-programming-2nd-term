			...
			public void actionPerformed(ActionEvent e) {		
				...
				/*else {
					if (rdbtnReverse.isSelected()) {
						...
						//output = java.lang.String.valueOf(list);
						output = list.toString();
						txtrOutput.setText(output);
						if (!flag) {
							Collections.reverse(list);
				...
				String QuantityList = "Количество элементов в списке: " + list.size();
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (index < 0 || index > list.size()) {
					txtrOutput.setText("Ошибка в вводе индекса\nИзмените индекс и повторите попытку");
					return;
				}
